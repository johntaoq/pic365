...
export async function requestVideoPricing({ providerId = '', seconds = 4 } = {}, { signal } = {}) {
  const params = new URLSearchParams({ providerId: String(providerId || ''), seconds: String(seconds || 4) });
...
export function useVideoPricing(input = {}, { enabled = true } = {}) {
  ...
  useEffect(() => {
    ...
    requestVideoPricing({ providerId, seconds }, { signal: controller.signal })

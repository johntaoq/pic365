...
function videoProviderDraft(provider = {}, imageProviders = []) {
  ...
  return {
    ...
    credentialSource: provider.credentialSource || 'image-provider',
...
export default function AdminVideoProvider({ language, session, imageProviders = [] }) {
  ...
  const previewDurations = draft.providerType === 'baidu-kling-video' ? [3, 5, 10, 15] : [4, 8, 12];
  const previewCredits = useMemo(() => Object.fromEntries(previewDurations.map((seconds) => [seconds, formatCredits(getVideoGenerationPricing({ seconds, mode: 'std' }, pricing).credits)])), [pricing, previewDurations.join(',')]);
  const manualCredentials = draft.credentialSource === 'manual';
  ...
  return (
    <section className="adminBlock adminProviderBlock adminChannelSection adminVideoProvider">
      ...
      <form className="adminProviderForm adminChannelEditor" onSubmit={save}>
        ...
        <label><span>{zh ? '渠道协议' : 'Provider protocol'}</span><select value={draft.providerType} onChange={(event) => { const providerType = event.target.value; setDraft((current) => ({ ...current, providerType, credentialSource: providerType === 'baidu-kling-video' ? 'manual' : current.credentialSource, name: current.id ? current.name : providerType === 'baidu-kling-video' ? '可灵 V3' : 'Sora 2', model: providerType === 'baidu-kling-video' ? 'kling-v3' : current.model, pricingConfig: providerType === 'baidu-kling-video' ? normalizeVideoPricingConfig({ ...current.pricingConfig, mode: VIDEO_PRICING_MODES.PER_SECOND, upstreamCurrency: 'CNY', upstreamPricePerSecond: 0.6, exchangeRate: 1, pricePerSecondRmb: 0.6, modeRatesRmb: { std: 0.6, pro: 0.8, '4k': 3 }, pricingSource: 'manual', pricingVersion: 'BK报价-2026-08-30' }) : current.pricingConfig })); }}><option value="openai-video-compatible">OpenAI / Sora</option><option value="baidu-kling-video">{zh ? '百度智能云 BK 可灵' : 'Baidu Cloud BK Kling'}</option></select></label>
        ...
        <label><span>{zh ? '密钥来源' : 'Credential source'}</span><select value={draft.credentialSource} disabled={draft.providerType === 'baidu-kling-video'} onChange={(event) => setDraft((current) => ({ ...current, credentialSource: event.target.value }))}><option value="image-provider">{zh ? '继承图片渠道' : 'Inherit image channel'}</option><option value="manual">{zh ? '独立配置' : 'Manual'}</option></select></label>
        {!manualCredentials ? <label><span>{zh ? '继承图片渠道' : 'Inherited image channel'}</span><select value={draft.imageProviderId} required onChange={(event) => { const inherited = imageProviders.find((provider) => provider.id === event.target.value); setDraft((current) => ({ ...current, imageProviderId: event.target.value, baseUrl: inherited?.baseUrl || '', apiKeyMasked: inherited?.apiKeyMasked || '' })); }}>{imageProviders.map((provider) => <option value={provider.id} key={provider.id}>{provider.name} · {provider.model}</option>)}</select></label> : null}
        <label><span>{zh ? '计费方式' : 'Billing mode'}</span><select value={pricing.mode} disabled={draft.providerType === 'baidu-kling-video'} onChange={(event) => updatePricing({ mode: event.target.value, pricingSource: 'manual' })}><option value={VIDEO_PRICING_MODES.PER_SECOND}>{zh ? '按秒计费' : 'Per second'}</option><option value={VIDEO_PRICING_MODES.PER_GENERATION}>{zh ? '按条计费' : 'Per generation'}</option></select></label>
        <label><span>Base URL</span><input value={manualCredentials ? draft.baseUrl : imageProviders.find((item) => item.id === draft.imageProviderId)?.baseUrl || draft.baseUrl} disabled={!manualCredentials} required onChange={(event) => setDraft((current) => ({ ...current, baseUrl: event.target.value }))} /></label>
        <label><span>API Key</span><input type="password" value={draft.apiKey || ''} placeholder={draft.apiKeyMasked || (manualCredentials ? (zh ? '留空则保留原密钥' : 'Leave blank to keep the current key') : (zh ? '继承加密 Key' : 'Inherited encrypted key'))} disabled={!manualCredentials} required={manualCredentials && !draft.apiKeyMasked} autoComplete="new-password" onChange={(event) => setDraft((current) => ({ ...current, apiKey: event.target.value }))} /></label>
        {draft.providerType !== 'baidu-kling-video' ? <label><span>{zh ? '每秒售价（元）' : 'Price per second (RMB)'}</span><input type="number" min="0" step="0.0001" value={pricing.pricePerSecondRmb} disabled={pricing.mode !== VIDEO_PRICING_MODES.PER_SECOND} onChange={(event) => updatePricing({ pricePerSecondRmb: event.target.value, pricingSource: 'manual' })} /></label> : null}
        {draft.providerType === 'baidu-kling-video' ? <>
          <label><span>720P · {zh ? '元/秒' : 'RMB/sec'}</span><input type="number" min="0" step="0.0001" value={pricing.modeRatesRmb?.std ?? 0.6} onChange={(event) => updatePricing({ modeRatesRmb: { ...pricing.modeRatesRmb, std: event.target.value }, pricingSource: 'manual' })} /></label>
          <label><span>1080P · {zh ? '元/秒' : 'RMB/sec'}</span><input type="number" min="0" step="0.0001" value={pricing.modeRatesRmb?.pro ?? 0.8} onChange={(event) => updatePricing({ modeRatesRmb: { ...pricing.modeRatesRmb, pro: event.target.value }, pricingSource: 'manual' })} /></label>
          <label><span>4K · {zh ? '元/秒' : 'RMB/sec'}</span><input type="number" min="0" step="0.0001" value={pricing.modeRatesRmb?.['4k'] ?? 3} onChange={(event) => updatePricing({ modeRatesRmb: { ...pricing.modeRatesRmb, '4k': event.target.value }, pricingSource: 'manual' })} /></label>
        </> : null}
        {draft.providerType !== 'baidu-kling-video' ? <label><span>{zh ? '每条售价（元）' : 'Price per video (RMB)'}</span><input type="number" min="0" step="0.01" value={pricing.pricePerGenerationRmb} disabled={pricing.mode !== VIDEO_PRICING_MODES.PER_GENERATION} onChange={(event) => updatePricing({ pricePerGenerationRmb: event.target.value, pricingSource: 'manual' })} /></label> : null}
        {draft.providerType !== 'baidu-kling-video' ? <label><span>{zh ? '美元汇率' : 'USD exchange rate'}</span><input type="number" min="0.0001" step="0.0001" value={pricing.exchangeRate} onChange={(event) => updatePricing({ exchangeRate: event.target.value, pricingSource: 'manual' })} /></label> : null}
        {draft.providerType !== 'baidu-kling-video' ? <label><span>{zh ? '上游美元/秒' : 'Upstream USD/second'}</span><input type="number" min="0" step="0.0001" value={pricing.upstreamPricePerSecond} onChange={(event) => updatePricing({ upstreamPricePerSecond: event.target.value, pricingSource: 'manual' })} /></label> : null}
        ...
        <div className="adminChatPricingMeta adminChannelPricingMeta">
          ...
          {previewDurations.map((seconds) => <span key={seconds}>{seconds}s：<b>{previewCredits[seconds]} {zh ? '积分' : 'credits'}</b></span>)}
        </div>

import { normalizeVideoDuration } from './video-generation.js';
...
export function getVideoGenerationPricing({ seconds = 4 } = {}, pricingConfig = {}) {
  const config = normalizeVideoPricingConfig(pricingConfig);
  const billedSeconds = normalizeVideoDuration(seconds);
  const rawRmb = config.mode === VIDEO_PRICING_MODES.PER_GENERATION
    ? config.pricePerGenerationRmb
    : config.pricePerSecondRmb * billedSeconds;
  ...
  return {
    ...
    pricePerSecondRmb: config.pricePerSecondRmb,
